...
import { state, activeTab, closeTab, toggleFocusMode, setFocusMode, STATUS_META, STATUS_LABEL } from '../store'
...
function locate(idx) {
  ...
  nextTick(() => {
    ...
    if (!el) return
    const target = el.querySelector(`.row[data-ri="${diffRows.value[clamped].i}"]`)
    if (!target) return
...
<template>
  <div class="col-center">
    ...
    <div class="filebar">
      ...
      <select class="form-select form-select-sm py-0" style="width:auto;font-size:.72rem"
              v-model="granularity" title="行内差异高亮粒度：整行 / 词 / 字符">
        <option value="line">整行</option>
        <option value="word">词级</option>
        <option value="char">字符级</option>
      </select>
      <button class="btn btn-sm py-0 px-2" style="font-size:.72rem"
      ...
      <div class="btn-group btn-group-sm ms-1" role="group" aria-label="差异行定位">
        <button class="btn btn-outline-secondary py-0 px-2" style="font-size:.72rem"
                @click="gotoPrev" :disabled="!diffRows.length" title="跳到上一处差异（Ctrl/Alt + ↑）">
          <i class="bi bi-chevron-up"></i>
        </button>
        <span class="btn btn-outline-secondary py-0 px-2 disabled d-flex align-items-center" style="font-size:.72rem;pointer-events:none">
    ...
    <div class="diff-area" :class="{ nowrap: !wrap }" v-if="node && dec && dec.ok" ref="diffAreaRef">
      <div class="diff-grid">
...
/* 折叠占位行 */
.row.fold { background: var(--bs-tertiary-bg); }
.row.fold .fold-ph { color: var(--bs-secondary-color); font-style: italic; text-align: center; cursor: pointer; user-select: none; }
...
.row.current { box-shadow: inset 3px 0 0 var(--bs-primary); }
/* 跳转动画：从主色高亮淡出，提示目标位置 */
.row.flash { animation: diffFlash .9s ease-out; }
@keyframes diffFlash {
