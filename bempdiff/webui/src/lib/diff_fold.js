...
// ctx 行：折叠模式下，仅保留落在某变化行（del/add/rep）前后 win 窗口内的；其余连续 ctx 折叠成一条占位行。
// 设计对标 Beyond Compare 的“忽略未变更区段”——收起噪声段、不隐藏真实代码。
//
// src: 解析后的行数组（每行 {type:'ctx'|'del'|'add'|'rep', ...}，rep=同行显示的修改对）
...
export function foldContext(src, collapse, win = 3) {
  ...
  const N = src.length
  let i = 0
  const isChange = (t) => t === 'del' || t === 'add' || t === 'rep'
  while (i < N) {
    const r = src[i]
    if (r.type !== 'ctx') {
      out.push({ ...r, _chg: chgSeq++ }) // del / add / rep：变化行，纳入跳转序列
      i++
      continue
    }
    if (!collapse) {
    ...
    for (let k = i - 1; k >= 0 && k >= i - win; k--)
      if (isChange(src[k].type)) { keep = true; break }
    if (!keep)
