...
## 五、差异依赖 JAR 内部源码对比（Top-15 内部 class 反编译源码级 diff）
...
### 阶段A 概览
- 整体风险：**MEDIUM**
- 影响范围：离线回放模式：未提供 stageA.response.txt，使用内置兜底摘要。差异文件数=11
- 测试主题：回归核心业务流程；校验对外接口兼容性；验证删除类无外部依赖
