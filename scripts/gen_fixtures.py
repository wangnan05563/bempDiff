...
产出：
  prototype/fixtures/small_real_v1.jar / v2.jar
      - 24 个真实 .class（取自 lib_v1.jar），其中 MODIFIED=4 / ADDED=2 / DELETED=2。
      - 用途：ai / report / export / 小批量 decompile 等需要"真实可反编译字节码"的端点。
  prototype/fixtures/big200_v1.jar / v2.jar   (~200MB, ZIP_STORED)
  prototype/fixtures/big500_v1.jar / v2.jar   (~500MB, ZIP_STORED)
      - 用真实 .class 复制到唯一路径，v2 对 ~2% 条目换用不同真实类字节（MODIFIED），
        其余 UNCHANGED。用途：解析 + 差异树 SLA（≤200MB <30s）与峰值压力。

真实字节码来源：prototype/lib_v1.jar（345 个真实编译类）。
"""
import os, random, zipfile, sys

ROOT = "prototype/fixtures"
SRC = "prototype/lib_v1.jar"
