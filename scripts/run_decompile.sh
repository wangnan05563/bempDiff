...
export JAVA_HOME="$(pwd)/prototype/toolchain/zulu21.52.15-ca-jdk21.0.12-win_x64"
...
"$PY" scripts/cfr_sampler.py logs/cfr_procs.csv &
SAMPLER=$!

run() {
  local name="$1"
  echo "===== START $name $(date +%T) ====="
  "$JM" -n -t "jmeter/$name.jmx" -Jhost=localhost -Jport=18080 -l "jmeter/results/$name.jtl" > "jmeter/results/$name.log" 2>&1
